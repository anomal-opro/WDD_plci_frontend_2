...
const PRESETS = [
  'Refill Gas LPG 3kg',
  'Refill Gas LPG 12kg',
  'Plastik Kresek & Bungkus',
  'Paper Box Takeaway',
  'Sabun Cuci Piring',
  'Es Batu Kristal',
  'Operasional & Parkir Pasar',
  'Token Listrik Tambahan',
  'Karet Gelang & Sedotan'
];

export default function BiayaVariabelView({ onDataChange, onOpenReceipt }) {
  ...
  // Load items where kategori === 'variabel'
  const items = useMemo(() => {
    const all = UnifiedExpenseDB.getAll();
    return all.filter(item => item.kategori === 'variabel');
  }, [dataVersion]);
  ...
  const handleCreate = (e) => {
    ...
    if (!namaItem || cleanNominal <= 0) {
      alert('Mohon isi uraian item dan nominal biaya yang valid!');
      return;
    }
    ...
    <div className="space-y-4 select-none">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
        <div className="relative flex-1 sm:w-72">
          ...
          <input
            type="text"
            placeholder="Cari biaya variabel..."
            value={search}
        ...
        >
          <Plus size={18} />
          <span>Catat Biaya Variabel Baru</span>
        </button>
      </div>
      ...
      <div className="hidden md:block bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
        <div className="p-4 sm:p-5 border-b border-slate-100 flex justify-between items-center">
          <h3 className="text-base font-black text-slate-900">Riwayat Pengeluaran Biaya Variabel</h3>
          ...
          <table className="w-full text-left text-xs whitespace-nowrap">
            <thead className="bg-slate-50 border-b border-slate-200 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="p-4">Item / Pengeluaran</th>
                <th className="p-4 text-right">Nominal Biaya</th>
              ...
              {filteredItems.length === 0 ? (
                <tr>
                  <td colSpan="6" className="p-10 text-center text-slate-400 font-bold">
                    Belum ada riwayat biaya variabel.
                  </td>
                </tr>
              ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl max-w-lg w-full p-5 sm:p-6 shadow-2xl border border-slate-200 animate-in slide-in-from-bottom sm:zoom-in-95 max-h-[92vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-4">
              <h3 className="text-base sm:text-lg font-black text-slate-900">Catat Biaya Variabel</h3>
              <button
            ...
            <form onSubmit={handleCreate} className="space-y-4 text-xs font-bold">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Contoh: Gas LPG 12kg, Kantong Kresek..."
                  value={namaItem}
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Contoh: Refill kompor dapur, stok seminggu..."
                  value={keterangan}
                  onChange={(e) => setKeterangan(e.target.value)}
                  className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 text-slate-800 outline-none focus:border-emerald-600"
                />
              </div>
